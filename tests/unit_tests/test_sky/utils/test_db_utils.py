...
class TestGetEngine:
    ...
    def test_postgres_async_engine_does_not_leak_libpq_kwargs_to_asyncpg(
            self, monkeypatch):
        ...
        with mock.patch('asyncpg.connect',
            ...
            async def trigger_connect():
                try:
                    ...
                    # SQLAlchemy will likely fail to use the mocked
                    # connection past the connect() call. That's fine —
                    # asyncpg.connect has already been invoked and the
                    # call args captured.
                    pass
